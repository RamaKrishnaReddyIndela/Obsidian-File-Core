...
async function extractBasicText(filePath, fileName) {
  try {
    ...
    if (/(\.txt|\.csv|\.json|\.log|\.md)$/i.test(lower)) {
      ...
      return raw.slice(0, 200000); // cap to 200k chars
    }
    return '';
  } catch {
    return '';
...
async function classifyFileAI({ filePath, fileName, text }) {
  ...
  if (/password|otp|one[- ]time|auth|token/.test(t)) scores.credentials += 0.3;
  if (/rs\.?|inr|account number|ifsc|upi|debit|credit|card|cvv|expiry/.test(t)) scores.financial += 0.3;
  if (/patient|diagnosis|blood|prescription|allergy|doctor/.test(t)) scores.health += 0.3;
  if (/function\s|class\s|import\s|require\(|def\s|public\s|package\s/.test(t)) scores.code += 0.3;
  ...
  const highIndicators =
    scores.credentials > 0.7 || scores.financial > 0.7 || scores.health > 0.7;
  if (highIndicators) risk_level = 'high';
  else if (top[1] > 0.4) risk_level = 'medium';
...
async function scanMalwareAI({ filePath }) {
  const KNOWN = new Set([
    '44d88612fea8a8f36de82e1278abb02f', // EICAR md5 demo
  ]);
...
module.exports = {
  classifyFile,     // ✅ main entrypoint for routes
  classifyFileAI,
  scanMalwareAI,
  detectPIIAI,
  extractBasicText,
